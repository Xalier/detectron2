...
__all__ = ["load_coco_gs_dir", "register_coco_gs_instances"]

def load_coco_gs_dir(root_dir):
    dataset_dicts = []
    timer = Timer()
    ...
    for instance_dir in instance_dirs:
        ...
        if len(label_file_ids) <= 0:
            logger.warn(f'Failed to find labels for for instance: {instance_dir}. Skipping.')
            continue
        ...
        for label_file_id in label_file_ids:
            ...
            try:
                ...
                logger.warn(f'Failed to find label file: {instance_dir}/output/{label_file_id}.txt. Skipping.')
                continue
...
def register_coco_gs_instances(name, metadata, root_dir):
    assert isinstance(name, str), name
    assert isinstance(root_dir, (str, os.PathLike)), root_dir
    # 1. register a function which returns dicts
    DatasetCatalog.register(name, lambda: load_coco_gs_dir(root_dir))
...
if __name__ == "__main__":
    ...
    dicts = load_coco_gs_dir(sys.argv[1])
